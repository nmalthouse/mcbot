...
pub fn emitPacketEnum(alloc: std.mem.Allocator, root: *std.json.ObjectMap, writer: anytype, game_state: []const u8, direction: []const u8, enum_name: []const u8) !void {
    ...
    if (write_types) {
        ...
        try parent.emit(writer, .{ .none = {} }, if (std.mem.eql(u8, "toClient", direction)) .recv else .send);
        try writer.print("}};\n", .{});
    }
    try writer.print("}};\n\n", .{});
}
                ...
                switch (emit_kind) {
                    .none => {
                        for (self.fields.items) |f| {
                            discard_psend = false;
                            switch (f.type) {
                                .primitive => |p| try w.print("try pk.{s}(self.{s});\n", .{ p, f.name }),
                                .compound => try w.print("try self.{s}.send(pk);\n", .{f.name}),
                            }
                        }
                    },
                    .array_varint, .array_count => {
                        discard_psend = false;
                        if (self.fields.items.len != 1) return error.invalidArrayStructure;
                        //const item_field = self.fields.items[0];
                        //const child_name = item_field.type.getIdentifier();
                        try w.print("for(self)|iter_item|{{", .{});
                        try w.print("try iter_item.send(pk);\n", .{});
